...
def process_csv(csv_path: str, is_original: bool = True) -> pd.DataFrame:
    '''Function for opening and processing a CSV annotation file'''
    if is_original:
        # Opening the original annotation CSV
        dframe = pd.read_csv(
            csv_path, delimiter=",", names=["Absolute path", "Relative path", "Class"]
        )
        dframe = dframe.drop("Relative path", axis=1)
    else:
        # Opening a regular CSV annotation
...
def create_annotation_file(annotation_path: str, dataset_path: str) -> None:
        try:
            if annotation_path and dataset_path:
                for class_name in ['cat', 'dog']:
                    abs_paths = get_absolute_paths(class_name, dataset_path)
                    rel_paths = get_relative_paths(class_name, dataset_path)
                    write_annotation_to_csv(annotation_path, abs_paths, rel_paths, class_name)

                QMessageBox.about(None, "Success", "Annotation file successfully created.")
        except Exception as ex:
            logger.log_error(f"Couldn't create annotation: {ex}")
            QMessageBox.critical(None, "Error", f"Couldn't create annotation: {ex}")
...
class MainWindow(QWidget):
    ...
    def create_csv(self, dataset_folder: str) -> None:
        try:
            ...
            create_annotation_file(self.annotation_path, dataset_folder)  # Используем новую функцию
        except Exception as error:
            logger.log_error(f"Failed to create annotation: {error}")
            QMessageBox.critical(None, "Error", f"Failed to create annotation: {error}")
        ...
        try:
            if self.dataset_path:
                ...
                if self.annotation_path:
                    create_annotation_file(self.annotation_path, self.dataset_path)  # Используем новую функцию
        except Exception as ex:
            logger.log_error(f"Couldn't create annotation: {ex}")
            QMessageBox.critical(None, "Error", f"Couldn't create annotation: {ex}")
    ...
    def copy_dataset(self, with_random: bool) -> None:
        try:
            ...
            QMessageBox.critical(None, "Error", f"Couldn't create dataset: {ex}")
