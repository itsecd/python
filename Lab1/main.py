...
def download(objects : list, folder : str, count_images : int) -> None:
    """
    Download images
    ...
    ----------
    object : list
        Список объектов для скачивания
    folder : str
        Путь гланой папки для скачивания
    count : int
    ...
    for object in objects:
        count = 0
        exec_count = 0
        list_images = make_list(object, URL, count_images)
        for flower_url in list_images:
            if count >= count_images : break
            try:
                src=flower_url['src']
                response = requests.get(src)
                create_folder(os.path.join(folder, object))
                with open(os.path.join(folder, object, f"{count:04}.jpg").replace("\\", "/"), "wb") as file:
...
def make_list(object : str, url : str, count : int) -> list:
    """
...
if __name__ == "__main__":
    ...
    download(OBJECTS, MAIN_FOLDER, COUNT)
