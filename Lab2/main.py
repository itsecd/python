...
if __name__ == '__main__':
    ...
    if namespace.mode == 'X_Y':
        split_csv_by_columns(input_file=namespace.path,
                             output_file_x=f'{os.path.join(namespace.output_dir)}/X.csv',
                             output_file_y=f'{os.path.join(namespace.output_dir)}/Y.csv')
    if namespace.mode == 'years':
        split_csv_by_years(input_file=namespace.path,
                           output_folder=f'{os.path.join(namespace.output_dir)}/years')
    if namespace.mode == 'weeks':
        split_csv_by_weeks(input_file=namespace.path,
                           output_folder=f'{os.path.join(namespace.output_dir)}/weeks')
