...
def copy_dataset(old_dir:str, tags:str, new_dir:str, name_csv:str) -> None:
    """Function copy images from tags folders to the dataset"""
    try:
        img_list=[]
        for tag in tags:
            count_files=len(os.listdir(os.path.join(old_dir,tag)))
            for img in range(count_files):
                b=os.path.abspath(os.path.join(old_dir,tag,f"{img:04}.jpg"))
                a=os.path.abspath(os.path.join(new_dir,f"{tag}_{img:04}.jpg"))
                shutil.copy(b,a)
                l=[[a,os.path.realpath(a),tag]]
                img_list+=l
